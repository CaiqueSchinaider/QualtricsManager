...
export default function Listscripts() {
  ...
  return () => {
    ...
    return(
        <>
    {viewSignal.signal ? (<PopUp preset={viewSignal.preset} text={viewSignal.scriptText} editScript={viewSignal.scriptEdit}  back={() => setViewSignal({
        signal: false,
        scriptText: '',
        scriptEdit: false,
        preset: 'view'
                ...
                transition={{ delay: i * 0.03 }}
                    ...
                    className={` ${width!  > 650 ? 'w-8/10' : 'w-19/20'}  shadow-2xl hover:scale-[99.2%] duration-200 shadow-[#000] min-h-20 min-w-100 h-20 border   border-schin-gray-strong rounded-2xl flex flex-row`}>
                        <div className="  w-11/20 h-full  flex justify-start items-center pl-5 select-none">
                            ...
                            <Text size={width!  > 1000 ? 'extra large' : width!  < 730 ? 'small' : 'medium'} className="font-tech w-8/10 text-schin-gray-light border-l-2 border-schin-gray-strong h-6/10 flex pl-5 justify-start items-center">{code.name}</Text>   
                        </div>
                        <div className={`${width!  > 1000 ? 'gap-8' : 'gap-4'} h-full select-none w-9/20 flex flex-row items-center justify-end pr-8 `}>
                            ...
                            {code.html ? (  
                                <button onClick={() => setViewSignal({
                                        ...
                                        preset: 'script'
                                        })} className=" flex justify-center items-center hover:scale-99 hover:invert-85 invert-55 duration-75">
                                    <img src="html.webp" className="w-8 h-8 cursor-pointer min-w-8 min-h-8"  />
                                </button>
                            ) : (null)} 
                                ...
                                <button  onClick={() => handleCopyScript(code.script) } className=" flex justify-center items-center hover:scale-99 hover:invert-85 invert-60 duration-75">
