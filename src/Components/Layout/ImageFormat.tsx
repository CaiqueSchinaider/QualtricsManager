    ...
    async function handleFormat() {
        ...
        if (formatI !== 0) {
            if( ext || imgWidth || imgHeight || reduceImg || resizeScale) {
                toast.error('Não é possivel realizar outras formtações juntamente com a inteligente')
                return
            }
        }

        if((imgHeight || imgWidth) && resizeScale !== 0) {
            toast.error('Não é possivel redimensionar imagem e diminuir propoção ao mesmo tempo')
            return
        }
    ...
    return (
        <section className="w-full h-full">
               ...
               <div className="flex items-center justify-center w-full">
                    <label className="flex flex-col items-center justify-center w-full max-w-md h-44 border-2 border-dashed border-schin-white rounded-2xl cursor-pointer transition-all duration-200">
                        ...
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            <svg className="w-8 h-8 mb-3 text-schin-white" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M7 16V4m0 0l-4 4m4-4l4 4m6 8v8m0 0l-4-4m4 4l4-4"/>
                            </svg>
            ...
            <div className="w-full h-30  mt-38 flex flex-row">
                ...
                <Button text="Webp" size="small" onChildClick={() => ext === 1 ? setExt(0) : setExt(1)} activate={ext === 1} />
                <Button text="png" size="small" onChildClick={() => ext === 2 ? setExt(0) :  setExt(2)} activate={ext === 2}/>
                <Button text="jpeg" size="small" onChildClick={() => ext === 3 ? setExt(0) :  setExt(3)} activate={ext === 3}/>
                </div>
                <div className="w-1/2 h-full  flex flex-row relative pt-10 justify-center gap-3 ">
                    <Text size="large" className="text-schin-white absolute top-0 ">
                        Redimensionar imagem
                    </Text>
                    <Input label="" size="medium" InputConfig={{placeholder: 'Largura em pixel...', onChange: (e) => setImgWidth(e.target.value) }}/>
                    <Input label="" size="medium" InputConfig={{placeholder: 'Altura em pixel...', onChange: (e) => setImgHeight(e.target.value)}}/>
                </div>
            </div>
            ...
            <div className="w-full h-30  mt-20 flex flex-row">
                ...
                <Button text="Manter qualidade" size="small" onChildClick={() => reduceImg === 1 ? setReduceImg(0) :  setReduceImg(1)} activate={reduceImg === 1} />
                <Button text="Deixar leve" size="small"  onChildClick={() => reduceImg === 2 ? setReduceImg(0) :  setReduceImg(2)}  activate={reduceImg === 2}/>
                <Button text="Reduzir muito" size="small"  onChildClick={() => reduceImg === 3 ? setReduceImg(0) :  setReduceImg(3)} activate={reduceImg === 3}/>
                </div>
                <div className="w-1/2 h-full  flex flex-row relative pt-10 justify-center gap-3">
                    <Text size="large" className="text-schin-white absolute top-0 ">
                        Diminuir proporção
                    </Text>
                        <Button text="25%" size="small" onChildClick={() => resizeScale === 1 ? setResizeScale(0) :  setResizeScale(1)} activate={resizeScale === 1}/>
                        <Button text="50%" size="small" onChildClick={() => resizeScale === 2 ? setResizeScale(0) : setResizeScale(2)} activate={resizeScale === 2} />
                        <Button text="75%" size="small" onChildClick={() => resizeScale === 3 ? setResizeScale(0) : setResizeScale(3)} activate={resizeScale === 3} />
                </div>
            </div>
