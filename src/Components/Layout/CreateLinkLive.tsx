...
function createLinkLive(): string {
  ...
  return url.toString();
    ...
    return (
        <>
        {signalCopy.signal ? (<PopUp preset="linklive" text={ signalCopy.resposta} linkToCheck={createLinkLive()} back={() => setSignalCopy({
            signal: false, 
            resposta: ''
        })}/>) : (null)}
        ...
        <section className="w-full h-full flex flex-col justify-start items-center select-none">
            ...
            <div className="w-full flex flex-col h-30  mt-10  border-b-2 border-schin-gray-medium pl-16">
            ...
            <div className=" w-full flex flex-row gap-5  justify-start items-center ">
                <Button text="owid" size="small" onChildClick={() => handleSetParams('owid')} block={signalProcessando.signal} activate={params.includes('owid')}  />
                <Button text="panelistid" size="small" onChildClick={() => handleSetParams('panelistid')}block={signalProcessando.signal} activate={params.includes('panelistid')}/>
                <Button text="age" size="small" onChildClick={() => handleSetParams('age')} block={signalProcessando.signal} activate={params.includes('age')}/>
                <Button text="gender" size="small" onChildClick={() => handleSetParams('gender')} block={signalProcessando.signal} activate={params.includes('gender')}/>
                <Button text="sel" size="small" onChildClick={() => handleSetParams('sel')}block={signalProcessando.signal} activate={params.includes('sel')} />
           ...
           <div className="w-full flex flex-col h-30  mt-10 border-b-2 border-schin-gray-medium pl-17">
            ...
            <div className="w-9/10 flex flex-col h-80  mt-15 ">
            ...
            <Text size="large" className="text-schin-white pb-2">
             ...
             <textarea placeholder="Faça uma observação que possa ser util para o PM..." readOnly={signalProcessando.signal} onChange={(e) => setObs(e.target.value) }  className="resize-none w-5/10 h-70 border-schin-gray-medium border rounded-2xl text-schin-white pl-5 pt-2"></textarea>
                    <div className=" w-1/2 h-7/10 justify-center items-center flex flex-col gap-5">
                                <Button text={signalProcessando.text} size="large" onChildClick={() => handleCallbackPrompt()} block={signalProcessando.signal} contrastStyle activate={signalProcessando.signal}/>
                                <Button text="Criar Somente Link" size="large" onChildClick={() => handleCopyLinkLive()}   block={signalProcessando.signal}  contrastStyle />
                    </div>
                </div>
            </div>
        </section>
